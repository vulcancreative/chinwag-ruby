...
As with most things, if it sounds too good to be true, it probably is. While Chinwag and Chinwag Ruby are heavily tested in multiple platforms, it is a C-based application, which could prove a bit odd in unknown environments. That said, it is built to be quite solid and resilient, using age-old standards, and hiccups should be far and few between (if at all).
